...
async function rB() {
    ...
    if (u) {
        try {
            ...
            const result = await performBypass(u);
            
            clearInterval(textInterval);
            loadingAnimation.style.display = 'none';
            
            if (result.success) {
                await displayBypassResult(result.data);
                await performAutoBypass(result.data);
            } else {
...
async function performBypass(url, attempts = 0) {
    if (attempts >= 10) {
    ...
    try {
        ...
        if (data.success) {
            ...
            return performBypass(url, attempts + 1);
        }
    } catch (error) {
        console.error('Error en el bypass:', error);
        return { success: false, error: error.message };
    }
}

async function performAutoBypass(result) {
    const url = extractUrl(result);
    if (url) {
        displayAutoBypassStatus('Iniciando auto-bypass...');
        const autoBypassResult = await performBypass(url);
        if (autoBypassResult.success) {
            await displayBypassResult(autoBypassResult.data, true);
            await performAutoBypass(autoBypassResult.data);
        } else {
            displayAutoBypassStatus('Auto-bypass fallido');
        }
    }
}
...
function displayAutoBypassStatus(message) {
    const statusDiv = document.getElementById('autoBypassStatus');
    if (!statusDiv) {
        const newStatusDiv = document.createElement('div');
        newStatusDiv.id = 'autoBypassStatus';
        document.getElementById('resultado').appendChild(newStatusDiv);
    }
    document.getElementById('autoBypassStatus').textContent = message;
}
...
Here's the updated JavaScript code with the improved bypass process:

```javascript project="OneCreatorX Scripts" file="main.js" type="react"
const WORKER_DOMAIN = 'https://bypass.api-x.site';
let sM = 0, sPP = 110, s = [], pA = 0, sO = [], fA = "todos";
let lastActionTime = 0;
let clientIdentifier = null;
...
async function rB() {
    ...
    if (u) {
        try {
            ...
            const initialBypassResult = await performBypass(u, 'Bypass inicial');
            if (initialBypassResult.success) {
                await displayBypassResult(initialBypassResult.result, 'Resultado del bypass inicial');
                const urlInResult = extractUrl(initialBypassResult.result);
                if (urlInResult) {
                    const autoBypassResult = await performBypass(urlInResult, 'Auto-bypass');
                    if (autoBypassResult.success) {
                        await displayBypassResult(autoBypassResult.result, 'Resultado del auto-bypass');
                    }
                }
            } else {
                displayBypassResult({ error: initialBypassResult.error || await getNotificationText('URL_PROCESS_FAILED') }, 'Error en el bypass');
            }
        } catch (e) {
            console.error('Error al obtener la respuesta:', e);
            displayBypassResult({ error: await getNotificationText('REQUEST_PROCESS_ERROR') }, 'Error en el proceso');
        } finally {
            document.getElementById('btnBypass').disabled = false;
        }
    } else {
...
async function performBypass(url, bypassType, maxRetries = 10) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            updateLoadingMessage(`${bypassType}: Intento ${attempt} de ${maxRetries}...`);
            const response = await fetch(`${WORKER_DOMAIN}/bypass?url=${encodeURIComponent(url)}`);
            const data = await response.json();
            ...
            if (data.success) {
                ...
                return { success: false, error: data.error || `${bypassType} fallido después de ${maxRetries} intentos` };
            }
        } catch (error) {
        ...
        await new Promise(resolve => setTimeout(resolve, 1000)); // Espera 1 segundo entre intentos
    }
}

function updateLoadingMessage(message) {
    const loadingElement = document.getElementById('bypassLoading');
    if (!loadingElement) {
        const resultDiv = document.getElementById('resultado');
        const loadingDiv = document.createElement('div');
        loadingDiv.id = 'bypassLoading';
        loadingDiv.innerHTML = `<p class="loading-message">${message}</p>`;
...
async function displayBypassResult(result, title) {
    const resultDiv = document.getElementById('resultado');
    const resultElement = document.createElement('div');
    resultElement.className = 'bypass-result';
    ...
    const copyButton = resultElement.querySelector('.copy-result');
    if (copyButton) {
        copyButton.addEventListener('click', async () => {
            try {
                await navigator.clipboard.writeText(copyButton.dataset.result);
                showNotification('RESULT_COPIED');
            } catch (err) {
                console.error('Error al copiar: ', err);
            }
